...

{
    ...
    "category": "section",
    "text": "A light interface to serial and multi-threaded Sequential Monte CarloPages = [ \"intro.md\", \"smcintegrals.md\", \"smcalgorithm.md\", \"smctheory.md\", \"smcve.md\", \"smcadaptive.md\", \"csmc.md\", \"impl.md\", \"performance.md\", \"smcinterface.md\", \"guide.md\", \"hmm.md\", \"bench.md\", \"refs.md\"]"
},
...

{
    ...
    "category": "section",
    "text": "We can alternatively define the sequence of measures mentioned above recursively by eta_1=gamma_1=M_1, \\[ \\etap \\propto \\gammap := (\\gamma{p-1} \\cdot G{p-1}) Mp, \\qquad p \\in \\{2, \\ldots, n\\}, \\] and for pin 1ldotsn, \\hat{\\eta}p \\propto \\hat{\\gamma}p := \\gammap \\cdot Gp$. It follows that one can write \\hat{Z}1 = \\eta1(G1)$ and hatZ_p = hatZ_p-1 eta_p(G_p) for p in 2 ldots n.We observe that gamma_p=hatgamma_p-1M_p, and this suggests the following diagram \\[ M1 = \\gamma1 \\overset{G1}{\\longrightarrow} \\hat{\\gamma}1 \\overset{M{2}}{\\longrightarrow} \\gamma{2} \\longrightarrow \\cdots \\longrightarrow \\hat{\\gamma}{p-1} \\overset{Mp}{\\longrightarrow} \\gammap \\overset{Gp}{\\longrightarrow} \\hat{\\gamma}p \\longrightarrow \\cdots \\longrightarrow \\gamman \\overset{Gn}{\\longrightarrow} \\hat{\\gamma}n, \\] where measures are obtained by weighting or mutation. Alternatively, we can view the sequence of probability measures using the same diagram, i.e. \\[ M1 = \\eta1 \\overset{G1}{\\longrightarrow} \\hat{\\eta}1 \\overset{M{2}}{\\longrightarrow} \\eta{2} \\longrightarrow \\cdots \\longrightarrow \\hat{\\eta}{p-1} \\overset{Mp}{\\longrightarrow} \\etap \\overset{Gp}{\\longrightarrow} \\hat{\\eta}p \\longrightarrow \\cdots \\longrightarrow \\etan \\overset{Gn}{\\longrightarrow} \\hat{\\eta}n, \\] but where the weighting steps also involve renormalization: eta_p oversetG_plongrightarrow hateta_p means hateta_p = (eta_p cdot G_p)  eta_p(G_p).The measures hatgamma_p and hateta_p are often referred to as the \"updated\" versions of gamma_p and eta_p, respectively."
},
...

{
    ...
    "category": "section",
    "text": "For some p in 1ldotsn, the particle approximation of eta_p(f) is obtained by constructing a particle approximation eta_p^N of eta_p and then calculating eta_p^N(f). Specifically, we define \\[ \\etap^N := \\frac{1}{N} \\sum{i=1}^N \\delta{\\zetap^i}, \\qquad p \\in \\{1,\\ldots,n\\} \\] and the particle approximation of eta_p(f) is then simply \\[ \\etap^N(f) = \\int f(x) \\etap^N({\\rm d}x) = \\frac{1}{N} \\sum{i=1}^N f(\\zetap^i). \\]Similarly, we define \\[ \\hat{\\eta}p^N:= \\frac{\\etap^N \\cdot Gp}{\\etap^N(Gp)}, \\qquad p \\in \\{1, \\ldots, n\\} \\] so that the particle approximation of \\hat{\\eta}p(f)$ is \\[ \\hat{\\eta}^Np(f) = \\int f(x) \\hat{\\eta}p^N({\\rm d}x) \\frac{\\sum{i=1}^N Gp(\\zetap^i) f(\\zetap^i)}{\\sum{i=1}^N Gp(\\zeta_p^i)}. \\]Finally, the particle approximations of hatZ_1ldotshatZ_n are defined by hatZ_1^N=eta_1^N(G_1) and for p in 2ldotsn, hatZ_p^N=hatZ_p-1^Neta_p^N(G_p)."
},

{
    ...
    "category": "section",
    "text": "The particle system defined by the SMC algorithm has a genealogical structure, induced by the ancestor indices. We define Eve indices such that E_p^i is the index of the time 1 ancestor of zeta_p^i. That is, the Eve indices satisfy E_1^i = i for i in 1 ldots N and \\[ Ep^i = E{p-1}^{A_{p-1}^i}, \\qquad p \\in \\{2, \\ldots, n\\}, \\quad i \\in \\{1, \\ldots, N\\}. \\] These can be used to quantify and visualize the path degeneracy phenomenon associated with the SMC algorithm. Since the ancestor indices are in sorted order, so too are the Eve indices at each time."
},

{
    ...
    "title": "Recursive definition",
    "category": "section",
    "text": "The SMC algorithm is intimately connected to the recursive definition of the SMC measures themselves.Let us define the sampling operator S^N to be the random map that takes a measure mu and outputs a random measure \\[ \\mu^N = S^N \\mu = \\frac{1}{N} \\sum{i=1}^N \\delta{X^i}, \\qquad \\text{where } X^i \\overset{\\mathrm{i.i.d.}}{\\sim} \\mu. \\]Then we have \\[ \\eta^N1 = S^N M1, \\qquad \\hat{\\eta}1^N = \\frac {\\eta^N1 \\cdot G1} {\\eta^N1(G1)}, \\] and for p = 2 ldots n, \\[ \\eta^Np = S^N (\\hat{\\eta}{p-1}^N Mp), \\qquad \\hat{\\eta}p^N = \\frac {\\eta^Np \\cdot Gp} {\\eta^Np(G_p)}. \\]This mirrors almost exactly the recursive definitions for the SMC measures, where one has instead \\[ \\eta1 = M1, \\qquad \\hat{\\eta}1 = \\frac {\\eta1 \\cdot G1} {\\eta1(G1)}, \\] and for p = 2 ldots n, \\[ \\etap = \\hat{\\eta}{p-1} Mp, \\qquad \\hat{\\eta}p = \\frac {\\etap \\cdot Gp} {\\etap(G_p)}. \\]We notice also that hatZ^N_p = hatZ^N_p-1 eta^N_p(G_p) while hatZ_p = hatZ_p-1 eta_p(G_p) for p in 2 ldots n."
},
...

{
    ...
    "category": "section",
    "text": "We recall that the particle approximations have asymptotic variances associated with particular test functions f. Letting n be arbitrary to avoid additional indexing notation, the values sigma_n^2(f) and hatsigma_n^2(f) are typically not possible to calculate exactly. They can be decomposed into sums as follows: \\[ \\sigman^2(f) = \\sum{p=1}^n v{p,n}(f), \\qquad \\hat{\\sigma}n^2(f) = \\sum{p=1}^n \\hat{v}{p,n}(f).\\] The quantities v_pn(f) and hatv_pn(f) also cannot be computed exactly in general, but this decomposition can in some circumstances shed some light on the nature of the approximation errors."
},
...
{
    "location": "performance.html#Performance-tips-1",
    "page": "Performance tips",
    "title": "Performance tips",
    "category": "section",
    "text": "The majority of the computation involved in running SMC is the computation of the mutation and log potential functions. The goal in the design of this package has been to allow users to specify these functions in ways that avoid unnecessary computations and memory allocations.For example, user-defined scratch space structs are intended to allow users to avoid any memory allocations entirely. Running the algorithm in serial is possible with no allocations whatsoever. Example scratch space structs are defined in the multivariate linear Gaussian model and Lorenz96 models in SMCExamples.It is sometimes possible to store important per-particle information as part of the data associated with a particle. This is done, e.g., in the SMC sampler demo in SMCExamples to store two different log densities of the state, which can therefore be computed only once.In general, use of StaticArrays.jl can provide dramatic improvements for multivariate particles, and is highly recommended for fixed-size vector components of particles."
},
...

{
    ...
    "category": "section",
    "text": "A vector of approximations (loghatZ_1^N ldots loghatZ_n^N) is stored in smcio.logZhats, which can be used in conjunction with SequentialMonteCarlo.eta to produce the approximations gamma_p^N(f) or hatgamma_p^N(f).One can extract the approximation eta_p^N(f) or hateta_p^N(f) by callingSequentialMonteCarlo.eta(smcio, f, hat::Bool, p::Int64)with hat determining which approximation is returned. Calling this function with p < smcio.n requires smcio.fullOutput = true.One can extract the approximations (eta^N_p(f) geq 0 log gamma^N_p(f)) or (hateta^N_p(f) geq 0 log hatgamma^N_p(f)) by callingSequentialMonteCarlo.slgamma(smcio, f, hat::Bool, p::Int64)with hat determining which approximation is returned. Calling this function with p < smcio.n requires smcio.fullOutput = true.If smcio.fullOutput = true one can call SequentialMonteCarlo.allEtas or SequentialMonteCarlo.allGammas to obtain a vector of outputs of SequentialMonteCarlo.eta or SequentialMonteCarlo.slgamma, respectively."
},
...
{
    "location": "guide.html#SequentialMonteCarlo.smc!-Tuple{SMCModel,SMCIO}",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.csmc!-Union{Tuple{Particle}, Tuple{SMCModel,SMCIO{Particle,ParticleScratch} where ParticleScratch,Array{Particle,1},Array{Particle,1}}} where Particle",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.SMCIO-Union{Tuple{Int64,Int64,Int64,Bool}, Tuple{Int64,Int64,Int64,Bool,Float64}, Tuple{ParticleScratch}, Tuple{Particle}} where ParticleScratch where Particle",
    "page": "Types and functions",
    "title": "SequentialMonteCarlo.SMCIO",
    "category": "method",
...
{
    "location": "guide.html#SequentialMonteCarlo.eta-Union{Tuple{F}, Tuple{Particle}, Tuple{SMCIO{Particle,ParticleScratch} where ParticleScratch,F,Bool,Int64}} where F<:Function where Particle",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.allEtas-Union{Tuple{F}, Tuple{SMCIO,F,Bool}} where F<:Function",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.slgamma-Union{Tuple{F}, Tuple{SMCIO,F,Bool,Int64}} where F<:Function",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.allGammas-Union{Tuple{F}, Tuple{SMCIO,F,Bool}} where F<:Function",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.V-Union{Tuple{F}, Tuple{Particle}, Tuple{SMCIO{Particle,ParticleScratch} where ParticleScratch,F,Bool,Bool,Int64}} where F<:Function where Particle",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.vpns-Union{Tuple{F}, Tuple{SMCIO,F,Bool,Bool,Int64}} where F<:Function",
    "page": "Types and functions",
...
{
    "location": "guide.html#SequentialMonteCarlo.v-Union{Tuple{F}, Tuple{SMCIO,F,Bool,Bool,Int64}} where F<:Function",
    "page": "Types and functions",
    "title": "SequentialMonteCarlo.v",
    "category": "method",
...
{
    "location": "bench.html#Benchmarks-1",
    "page": "Benchmarks",
    "title": "Benchmarks",
    "category": "section",
    "text": ""
...
{
    "location": "bench.html#E5-2667-v3-(Haswell)-1",
    "page": "Benchmarks",
    "title": "E5-2667 v3 (Haswell)",
    "category": "section",
